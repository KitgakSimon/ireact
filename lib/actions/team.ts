    ...
    if (data.email) {
      ...
      if (!existingUser) {
        ...
        try {
          await sendEmail({
            to: data.email,
            subject: "Welcome to the IREACT Admin Team! 🔐",
            html: `
              <div style="font-family: sans-serif; max-width: 600px; margin: auto; padding: 30px; border: 1px solid #e2e8f0; border-radius: 24px; color: #1e293b; background-color: #ffffff;">
                ...
                <div style="background-color: #f8fafc; padding: 25px; rounded: 20px; border-radius: 20px; margin-bottom: 30px;">
                  <h2 style="font-size: 18px; font-weight: 800; color: #0f172a; margin-top: 0;">Welcome, ${data.name}!</h2>
                  <p style="font-size: 15px; color: #475569; line-height: 1.6;">You have been added as a team member and granted administrative access to the IREACT dashboard. You can now manage blog posts, team members, and site content.</p>
                </div>
